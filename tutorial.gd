...
var dialogue := {
	"intro_1": {
		"speaker": "Jed",
		"text": "Привет, спасибо, что во время хайпа силксонг нашёл время на пробную версию oneshooters :) ",
		"next": "intro_2"
	},
	"intro_2": {
		"speaker": "Jed",
...
func handle_action(action: String):
	match action:
		...
		"enable_reloading":
			...
			var enemy = preload("res://enemy.tscn").instantiate()
			var fire = preload("res://Fire_Target_Visual.tscn").instantiate()
		...
		"spawn_shooting_enemy":
			...
			var enemy = preload("res://shooting_enemy.tscn").instantiate()
		...
		"dodge":
			...
			var drone = preload("res://drone_bonus.tscn").instantiate()
